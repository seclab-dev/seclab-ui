...
- `SecLabLoading` 的覆盖模式建立稳定层叠上下文，避免表格粘性表头或固定列显示在加载遮罩之上。

## [0.1.0-alpha.8] - 2026-08-09

### Fixed

- `SecLabActionMenu` 在同一页面只保留一个展开菜单，并在浮层完成定位后再显示，避免首次打开时页面滚动条闪烁。
...
- 统一受控输入、Menu、Tabs、Tooltip 与 ActionMenu 的跨框架公开契约，移除重复属性、事件和配置字段别名。
